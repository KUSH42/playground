...
export class GlobeComponent implements AfterViewInit {
  ...
  private setupOrbitControls() {
    this.camera.orbitControls = new OrbitControls(this.camera.object, this.canvas.nativeElement);
  ...
  private focusUser() {
    if (this.state.users.length > 0) {
      if (this.state.currentUserIndex === null) {
        ...
        this.state.currentUserIndex = this.getRandomNumberBetween(0, this.state.users.length - 1)
      } else {
        ...
        this.state.currentUserIndex = (this.state.currentUserIndex + 1) % this.state.users.length
      }
